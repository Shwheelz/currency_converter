...
export function changeDestAmount(newDestAmount) {
    return {
        type: types.CHANGE_DESTINATION_AMOUNT,
        data: {
            newDestAmount: newDestAmount
        }
    }
}
...
export function changeDestCurrency(newDestCurrency) {
    return {
        type: types.CHANGE_DESTINATION_CURRENCY,
        data: {
            newDestCurrency: newDestCurrency
        }
    }
}
...
function _makeFeeAjaxCall(payload, dispatch) {
    ...
    .catch((resp) => {
        ...
        dispatch({type:types.RECEIVED_FEES_FAILURE, data: {msg: msg, failedCall:'fees'}});
    });
}
